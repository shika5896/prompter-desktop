...
### メニュー画面
編集・表示・設定の3つのボタンが表示されるトップ画面。

### 編集画面
- 左サイドバーにスライド一覧（キーボード操作・複製対応）
- ツールバーでフォントサイズ・色の変更、振り仮名追加/削除/自動付与、Undo/Redo
- テキストエディタで直接編集、振り仮名はクリックでダイアログ編集
- プレビューパネルで表示画面と同じ見た目を確認
...
│   ├── screens/                #   4画面（Menu, Edit, Display, Settings）
│   ├── components/             #   UIコンポーネント
│   │   ├── common/             #     汎用（Button, Modal, ConfirmDialog等）
...
└── src-tauri/                  # バックエンド（Rust）
    └── src/
        ├── commands/           #   Tauriコマンド（TOML読み書き, 自動振り仮名）
        └── models/             #   データモデル（原稿, 設定）
```
